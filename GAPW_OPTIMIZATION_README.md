...
该优化的目标是提升模型对局部表情区域的建模能力，并增强模型在遮挡场景下的鲁棒性。需要注意的是，本次优化的核心不是输出级局部头融合，也不是测试时增强 TTA，而是网络结构内部的局部区域自适应加权机制。
...
输出级局部头融合和 TTA 可以作为工程实验观察或补充分析，但不建议作为本文的主要创新点。本文主要创新点应放在 GAPW 模块及其 patch 可靠性建模上。
